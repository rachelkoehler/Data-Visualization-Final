...
stheme <- theme_classic() + 
  ...
  theme(axis.text.x = element_text(size = 12, colour = "black"), # customize x-axis text
        axis.title.y = element_text(size=12, angle=90, colour="black", face="bold"), # customize y-axis title
        axis.text.y = element_text(size = 12, colour="black"), # customize y-axis text
...
amp <- ggplot(subset(ur, gender=="Male"), 
  ...
  scale_y_continuous(limits = c(0, 35), breaks = c(0, 10, 20, 30), sec.axis=dup_axis()) + # set y-axis breaks and add secondary axis
...
bmp <- ggplot(subset(ur, gender=="Male"), 
  ...
  geom_point(data=subset(ur, gender=="Male" & (year==1999 | year==2018) & 
             ...
             fill=nhbc, size=3, stroke=2) + # add points
  scale_y_continuous(limits = c(0, 35), breaks = c(0, 10, 20, 30), sec.axis=dup_axis()) + # set y-axis breaks and add secondary axis
  scale_x_continuous(breaks=c(2005,2015), sec.axis=dup_axis()) + # set x-axis breaks and add secondary axis
  xlab("") + ylab("") + stheme +
...
wmp <- ggplot(subset(ur, gender=="Male"), 
  ...
  geom_point(data=subset(ur, gender=="Male" & (year==1999 | year==2018) & 
             ...
             fill=nhwc, size=3, stroke=2) + # add points
  scale_y_continuous(limits = c(0, 35), breaks = c(0, 10, 20, 30), sec.axis=dup_axis()) + # set y-axis breaks and add secondary axis
  scale_x_continuous(breaks=c(2005,2015), sec.axis=dup_axis()) + # set x-axis breaks and add secondary axis
  xlab("") + ylab("") + stheme +
  theme(axis.line.y = element_blank(), # remove left y-axis line
...
hmp <- ggplot(subset(ur, gender=="Male"), 
  ...
  geom_point(data=subset(ur, gender=="Male" & (year==1999 | year==2018) & 
             ...
             fill=hc, size=3, stroke=2) + # add points
  scale_y_continuous(limits = c(0, 35), breaks = c(0, 10, 20, 30), sec.axis=dup_axis()) + # set y-axis breaks and add secondary axis
  scale_x_continuous(breaks=c(2005,2015), sec.axis=dup_axis()) + # set x-axis breaks and add secondary axis
  xlab("") + ylab("") + stheme +
  theme(axis.line.y = element_blank(), # remove left y-axis line
...
awp <- ggplot(subset(ur, gender=="Female"), 
  ...
  geom_point(data=subset(ur, gender=="Female" & (year==1999 | year==2018) & 
                           racen=="Non-Hispanic\nAPI"), shape=21, colour="white",
             fill=nhac, size=3, stroke=2) + # add points
  scale_y_continuous(limits=c(0,20), breaks=c(0,10,20), sec.axis=dup_axis()) + # set y-axis limits and breaks
  scale_x_continuous(breaks=c(2005,2015), sec.axis=dup_axis()) + # set x-axis breaks and add secondary axis
  xlab("") + ylab("") + stheme +
...
hwp <- ggplot(subset(ur, gender=="Female"), 
              aes(x=year, y=aadr, group=racen))  + 
  geom_line(show.legend=F, colour="grey") + # plot all groups in grey
  geom_line(data=subset(ur, gender=="Female" & racen=="Hispanic"), # highlight Hispanic in purple
            colour=hc, size=1.5) +
  geom_point(data=subset(ur, gender=="Female" & (year==1999 | year==2018) & 
                           racen=="Hispanic"), shape=21, colour="white", 
             fill=hc, size=3, stroke=2) + # add points
